...
switch ($mode) {
  ...
  $pt_asal = $purifier->purify($_POST["pt_asal"]);
     ...
     case 3:
          ...
          ;
           $skld= upload_for_mahasiswa("skld", "$kode", 1, $path_upload);
          ;
           $tgl_kitas= $UTILITY->format_tanggal_db($_POST["tgl_kitas"]);
           $tgl_skld= $UTILITY->format_tanggal_db($_POST["tgl_skld"]);
...
if ($kondisi == "account") {
     ...
     /*      $DB->query("update user set password='$password',"
      ...
      $UTILITY->location_goto("content/ekstension/permit/$username");
}
if ($kondisi == "tambah") {
     $next = $mode + 1;
     ...
     $UTILITY->location_goto("content/ekstension/edit/$id_next/$next");
} else if ($kondisi == "edit") {
     $next = $mode + 1;
     $MHS->updateMahasiswa($data, $mode);
     $UTILITY->location_goto("content/ekstension/edit/$kode/$next");
}
if ($hapusmahasiswa != "") {

     $MHS->deleteMahasiswa($hapusmahasiswa);
     $UTILITY->location_goto("content/ekstension");
}
?>
